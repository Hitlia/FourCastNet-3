...
# helpers
from common.layers import DropPath, LayerScale, MLP, EncoderDecoder
from common.spectral_convolution import SpectralConv
from features import get_water_channels, get_channel_groups
...
class AtmoSphericNeuralOperatorNet(nn.Module):
    ...
    def encode(self, x):
        ...
        """
        batchdims = x.shape[:-3] # batch_size c lat lon
    ...
    def decode(self, x):
        ...
        x_out = torch.cat([x_atmo.reshape(*batchdims, -1, *x_atmo.shape[-2:]),x_surf.reshape(*batchdims, -1, *x_surf.shape[-2:])],dim=-3)
        ...
        #     x_surf = x[..., -self.surf_embed_dim :, :, :]
        #     x_surf = self.surf_decoder(x_surf)
        #     shapedims = x_surf.shape[-2:]
        #     x_surf = x_surf.reshape(*batchdims,-1,*shapedims)
        #     x_out[...,-5:,:,:] = x_surf
        #     # x_out[..., self.surf_channels, :, :] = x_surf
    ...
    def forward(self, x):
        # x: batch_size (c_air*level_air + c_surf + c_aux) lat lon
        ...
        x = self.clamp_water_channels(x) #x: batch_size (c_air*level_air + c_surf) lat lon
